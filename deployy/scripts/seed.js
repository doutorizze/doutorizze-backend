...
const seedDatabase = async () => {
  try {
    ...
    const clinicExists = await query('SELECT id FROM users WHERE email = ?', ['clinica@exemplo.com']);
    
    if (clinicExists.length === 0) {
      const [clinicUser] = await query(`
        INSERT INTO users (email, password, name, role, status, email_verified)
        VALUES (?, ?, ?, ?, ?, ?)
      `, ['clinica@exemplo.com', clinicPassword, 'Dr. João Silva', 'clinic', 'active', true]);
    ...
    if (patientExists.length === 0) {
      await query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `, ['paciente@exemplo.com', patientPassword, 'Maria Santos', '(11) 88888-8888', 'patient', 'active', true]);
